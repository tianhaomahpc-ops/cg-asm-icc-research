...
- **要真赢**:① **只要电极输出**(取 Z 的电极行 = 互易 lead-field)→ apply 只是几个点积(~µs)、对任意 RHS 精确、离线只需"电极数"次伴随解 —— 压倒性且现成;② **要全躯干体积场** → 必须 **H-matrix/FMM 压缩** Z(远场块低秩、近界面不压)+ 用 BLAS,并把 250s 摊到真实 1e4–1e5 步(且 apply 得先压到 < 求解才谈得上摊销)。
