...
\begin{frame}{Mesh: conforming unstructured tetrahedra (heart-in-torso)}
\begin{columns}[T]
\begin{column}{0.54\textwidth}
\textbf{Geometry}: a heart slab $20\times7\times3$ mm centered inside a $50^3$ mm torso; the interface is \textbf{conforming} (Gmsh \texttt{BooleanFragments}, sharing the same interface nodes).\\[0.4em]
\textbf{Discretization}: continuous P1 Galerkin (MFEM 4.9), anisotropic $\sigma$ (fibers $\parallel x$), units mm/ms/mV/mS$\cdot$mm$^{-1}$.
\begin{table}\small
\begin{tabular}{@{}lrrr@{}}
\toprule
mesh & nodes (dof) & tets & bdr tris \\
\midrule
heart & $10\,085$ & $47\,582$ & $9\,510$ \\
torso & $36\,229$ & $206\,647$ & $12\,112$ \\
\midrule
\multicolumn{4}{@{}l}{interface shared dofs: $4\,757$ (coord-matched, partition-independent)}\\
\bottomrule
\end{tabular}
\end{table}
\end{column}
\begin{column}{0.44\textwidth}
\IfFileExists{fig_mesh.png}{\includegraphics[width=\linewidth]{fig_mesh.png}}%
{\centering\vspace{2em}\fbox{\parbox{0.9\linewidth}{\centering \texttt{fig\_mesh.png}\\(heart slab in torso, $y{\sim}0$ slice)}}\vspace{2em}}
\end{column}
\end{columns}
\vspace{0.3em}
\footnotesize Subdomains $=$ MPI ranks (METIS geometric partition); $n_p{=}$ \#ranks $=$ \#subdomains.
\end{frame}
