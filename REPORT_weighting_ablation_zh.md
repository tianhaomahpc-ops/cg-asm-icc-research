...
**12.5 "内 + 外"同时加权?——恒等式与剩余自由度的探测**

追问:按子域对角放进求和号内、再在外面夹全局对角 $G$,会不会更好?
**数学上不产生新方法族**:全局对角与限制算子交换($R_iG=\hat G_iR_i$),故
$G\big(\sum_iR_i^\top D_i\tilde A_i^{-1}D_iR_i\big)G=\sum_iR_i^\top(\hat G_iD_i)\tilde A_i^{-1}(D_i\hat G_i)R_i$
——"内+外"恒等于"纯内"配乘积权重;且 scheme 7 本来就是"内(ε 形状)×外(归一化子)"。
剩余的真自由度只有两个,均已实测(Sys3,nx=48):

(a) **外层 $G$ 偏离精确归一化子**($\alpha$ 探测:$d_i=[1{:}\varepsilon]/(1+(m{-}1)\varepsilon^2)^{\alpha/2}$,
$\alpha{=}1$ 即 scheme 7;$\alpha{>}1$ 在质上覆盖"外面再乘 $1/\sqrt m$"):

| $\alpha$($\varepsilon{=}0.7,O3$) | 0.6 | 0.8 | **1.0** | 1.2 | 1.4 |
|---|---|---|---|---|---|
| $L0$ / $L2$ | 105/59 | 98/61 | **98/61** | 101/62 | 111/64 |

$\alpha=1$ 附近平坦、两侧变差 ⇒ **外层取精确归一化子已是局部最优,额外的外层自由度买不到东西**。

(b) **内层形状从两级阶跃改为按层几何递减**($w\propto q^{\ell}$,$\ell$=BFS 图距 owner 核心的层深
≡ 第 $\ell$ 轮 MatIncreaseOverlap 层;最外层=贴人工边界=污染最重,被压最狠;
精确全局重归一 $\sum_i w_i^2=1$;`-pugrade q`。
锚点:$q{=}1$ 精确复现 sASM 132/104/103;$O{=}1$ 时 graded $q$ ≡ flat $\varepsilon{=}q$,100/66 ✓):

| $q$($O3,L2$) | 0.4 | 0.5 | 0.6 | 0.7 | **0.8** | 0.9 | flat $\varepsilon^*$ | sASM | RAS |
|---|---|---|---|---|---|---|---|---|---|
| iter | 72 | 66 | 62 | 59 | **58** | 61 | 61 | 66 | 51 |

**分层渐变确实更好**:58 vs flat 61 vs sASM 66(solve-only 同批:0.118 / 0.125 / 0.138 s)。
物理解释:flat ε 对"第 1 层(轻污染)"和"最外层(重污染)"一视同仁;
按深度递减把掩蔽集中在真正污染重的外层、保留近核心层的输入信息——正是机理所指。
对 RAS(51)的差距:sASM 落后 15 步,flat 收回 5(33%),graded 收回 8(53%),
**仍剩 ~47% 收不回——对称税的硬核部分**。
(graded 模式暂为 MFEM 端探针,未移植 PETSc 端;flat 模式已 18/18 逐位一致。)

**12.6 结论与出路**:对称 + 纯 Dirichlet 局部解 + 权重掩蔽 = 收益封顶(flat ~7%,graded ~12%)。
...
- graded/sMRAS 暂为 MFEM 端实现;移植 PETSc 端为后续工作。

### 13.4 跨系统验证(Sys1 / Sys2 / Sys3)

同一口径(warm solve-only,best-of-5,同批参照;nx=48,4 ranks)。
每格 = 最优配置 · 外层迭代 · 时间:

| 方法 | Sys1(dt=1e-2) | Sys2(pure Neumann 奇异) | Sys3(Laplace 1D+5N) |
|---|---|---|---|
| **graded+Cheby** | $O3L1,d2$ · 9 · 0.036 | $O3L1,d3$ · 28 · 0.095 | $O3L1,d2$ · 38 · **0.099** |
| **sMRAS** | $O3L2$ · **7** · 0.041 | $O3L2$ · **27** · **0.086**\* | $O3L2$ · 36 · 0.118 |
| sASM+Cheby(旧最优对称) | $O3L1,d2$ · 11 · 0.044 | $O2L1,d2$ · 42 · 0.106 | $O2L1,d2$ · 48 · 0.120 |
| sASM | 18 · 0.041 | 54 · 0.111 | 73 · 0.124 |
| RAS+GMRES | 15 · **0.026** | 40 · **0.074** | 51 · 0.112 |

(\*sMRAS 0.086 为 Sys2 对称类最优;RAS+GMRES 0.074 仍是 Sys2 全场最快。)

**结论**:
1. **两个新方法在三个系统上都是对称/CG/低内存类的冠军**:相对旧最优对称方法
   (sASM+Cheby)提速 Sys1 ~18%、Sys2 ~19%、Sys3 ~17%;**外层迭代在三系统都全场最少**
   (sMRAS:7/27/36;对比 RAS+GMRES:15/40/51)。
2. **graded+Cheby 在病态端(Sys3)拿下全场时间冠军**(0.099 vs RAS+GMRES 0.112);
   在良态端(Sys1/Sys2)RAS+GMRES 仍最快(每步最便宜:单次块解+轻 scatter)——
   与"复合方法的每步加价只在迭代昂贵时回本"的规律一致。
3. **sMRAS 的 SPD(能量收缩)在三个系统全成立**,包括奇异的 Sys2(真残差 ~1e-6,
   全网格 27–46 步无一发散)——把"RAS+ICC 能量范数收缩"的实证从 Sys3 扩展到
   质量主导与奇异系统,加强了其作为可证命题的候选地位。
4. 选型更新:**病态/大 dt** → graded+Cheby(全场最快);**内存受限+任意条件数** →
   sMRAS(对称类迭代最少、Sys2 对称类最快)或 graded+Cheby;**内存充裕+良态** →
   RAS+GMRES 仍最快。
