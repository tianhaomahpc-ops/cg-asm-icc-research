...
#### 1.5.1 在 SMRAS 基础上、**同等通信**下继续改进(实测阶梯 + SPD 余量)

既然 SMRAS 好在"restricted PU 消 over-count → $\lambda_{\max}=1$",**同等通信**(不加 overlap、不加全局求解、halo 交换次数不变)下继续改进 = 拉它**没碰的轴**。三根杠杆,实测(Sys2,overlap=2,均 `CONVERGED_RTOL`,见 `cardiac/figs/fig_smras_ladder.png`):

| 配置 | 迭代 | 这一步代价 | 拉的轴 |
|---|---|---|---|
| sASM(基线) | 79 | — | — |
| SMRAS(theta=1, ICC0) | 45 | 同通信(2× halo) | over-count → $\lambda_{\max}=1$ |
| **+ theta=1.6**(过松弛) | **36** | **免费**(1 个标量) | 收缩因子 |
| **+ ICC(2)**(theta=1.5) | **24** | 同通信 + 本地 flops(fill ×2.9) | 局部精度 |
| **+ Cheby(2)**(替代 ICC2) | **24** | 同通信 + 本地**并行** flops,**低内存** | 局部精度(大规模首选) |

谱机制:SMRAS 不只 $\lambda_{\max}\to1$,还把 $\lambda_{\min}$ 从 sASM 的 3.75e-3 翻到 **7.66e-3**(κ 331→130);`iters/√κ` 从 4.34 掉到 **3.94**,说明 SMRAS 的谱**还更聚集**(PU 把谱往 1 堆),超出"光看 κ"的预测——即**谱分布**也在帮忙,不只极值。
...
**并行版做出来了(`cardiac/soras_par.cpp`),并大幅超过生产基线。** 1 子域/rank 的并行 SORAS:**rank-local Neumann 块**免费取自 `a.SpMat()`(本地单元装配,共享 dof 处天然 Neumann);**真 $M_\Gamma$ 在 rank 间的 shared face 上装配**(`GetSharedFace`/`GetFaceTransformation`,带界面非对角耦合);PU 由 $P^T\mathbf1$ 求重数得到;对称组合 $M^{-1}=P^T D (K_{\rm loc}+\alpha M_\Gamma)^{-1} D P$,$P^T$ 完成唯一一次 rank 间通信(= 一次 halo)。实测真实心脏 Sys2(纯 Neumann 各向异性,10085 dof,MFEM 并行 CG):

| ranks | block-Jacobi+ICC(现 `forward_ecg` 基线) | **并行 SORAS** | 降幅 |
|---|---|---|---|
| 2 | 90 | **27** | −70% |
| 4 | 92 | **41** | −57% |
| 8 | 93 | **47** | −49% |

α 扫描(4 ranks):最优 α≈0.1 → 41,大 α(界面过钉 ≈ Dirichlet 极限)→ 151,印证优化传输的最优点。**并行 SORAS 比现生产基线(bjacobi)少 ~50–70% 迭代,同样一次 rank 间通信/步。** 图:`cardiac/figs/fig_soras_par.png`。(注:相对 bjacobi 的大幅提升里,PU + Neumann 块本身贡献一部分,Robin 传输在其上再优化;两者都在同通信预算内。)
