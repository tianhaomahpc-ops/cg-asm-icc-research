...
\framesubtitle{$\eta=\mathrm{avg}_i\|A_i u_i-v\|/\|v\|$, the inexactness proxy (one ICC solve of a random $v$; $\eta\approx0$ = exact)}
\begin{itemize}\itemsep0.35em\footnotesize
  \item \textbf{(b) the headline.} 1D \emph{natural} \& \emph{RCM}: $\eta\approx10^{-15}$ --- ICC \emph{is} exact Cholesky, because a tridiagonal has \emph{no fill}, so dropping fill drops nothing. 1D \emph{random}: $\eta=0.67$ --- the permutation creates fill that ICC(0) throws away, so ICC is now inexact.
