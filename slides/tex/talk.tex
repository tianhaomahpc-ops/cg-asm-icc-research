...
\textbf{机制.}\ 预条件 CG 迭代数 $\sim\tfrac12\sqrt{\kappa}\,\ln(2/\varepsilon)$,单层加性 Schwarz 有
\[
\kappa(M^{-1}A)\ \lesssim\
\underbrace{C_0^{2}(\delta)}_{\text{随 overlap }\downarrow}\ \cdot\
\underbrace{N_c}_{\text{(B) 重叠重复计数}}\ \cdot\
\underbrace{\omega_{\max}/\omega_{\min}}_{\text{(A) 子域不精确,精确时}=1}
\]
\begin{center}
\begin{tabular}{l ccc c}
\toprule
