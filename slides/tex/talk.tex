...
\setbeamertemplate{footline}{\hfill\footnotesize\color{Gray}\ifnum\insertframenumber>14 附录\else\insertframenumber\,/\,14\fi\hspace{0.4cm}\vspace{3pt}}
...
% ===== S5c : ASM vs sASM on Sys1 / Sys2 (universality of anomaly + fix) =====
\begin{frame}{普遍性:Sys1 / Sys2 上的 ASM vs sASM}
\framesubtitle{迭代次数;$n_x{=}48$,4 ranks,METIS,rtol$=10^{-6}$;子域 ICC($L$),外层 CG}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\textbf{Sys1:reaction--diffusion}\ \footnotesize($\tfrac1{\Delta t}M{+}\tfrac12K$,$\Delta t{=}10^{-2}$)
\vskip0.2em
\begin{center}\footnotesize\setlength{\tabcolsep}{5pt}
\begin{tabular}{c cc cc}
\toprule
 & \multicolumn{2}{c}{$L=0$} & \multicolumn{2}{c}{$L=2$} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
$O$ & BASIC & sASM & BASIC & sASM \\
\midrule
0 & 33 & 33 & 24 & 24 \\
1 & 42 & \textbf{27} & 19 & \textbf{16} \\
2 & 47 & \textbf{27} & 20 & \textbf{15} \\
3 & — & 27 & — & 14 \\
\bottomrule
\end{tabular}
\end{center}
\vskip0.2em
{\footnotesize 心脏量级 $\Delta t{=}10^{-4}$:BASIC $6\to8\to9$,sASM $6\to4\to4$。\\
最优 solve-only:BASIC 0.048 s vs sASM \textbf{0.041 s}。}
\end{column}
\begin{column}{0.5\textwidth}
\textbf{Sys2:全 Neumann(奇异)}\ \footnotesize($\ker=\mathrm{span}\{1\}$,MatNullSpace$+$RHS 去均值)
\vskip0.2em
\begin{center}\footnotesize\setlength{\tabcolsep}{5pt}
\begin{tabular}{c cc cc}
\toprule
...
\midrule
0 & 102 & 102 & 77 & 77 \\
1 & 110 & \textbf{75} & 67 & \textbf{59} \\
2 & 134 & \textbf{74} & 65 & \textbf{54} \\
3 & — & 74 & — & 53 \\
\bottomrule
\end{tabular}
\end{center}
\vskip0.2em
{\footnotesize 奇异系统也发病($102\to134$)、也被修复。\\
最优 solve-only:BASIC 0.121 s vs sASM \textbf{0.110 s}。}
\end{column}
\end{columns}
\vskip0.4em
\textbf{观察.}\ 与 Sys3 一致:BASIC 随 $O$ 上升(反常)、sASM 随 $O$ 下降(修复);$O{=}0$ 两者相同($D{=}I$)。反常与修复均与边界/算子类型无关。
\end{frame}
...
% ===== S9c : cross-system four-method optima =====
\begin{frame}{跨系统验证:四方法在 Sys1 / Sys2 / Sys3 上的最优}
\framesubtitle{$n_x{=}48$,4 ranks;每格 = 最优配置 · 外层迭代 · solve-only 时间(warm,best-of-3/5)}
\begin{center}\footnotesize\setlength{\tabcolsep}{5pt}
\begin{tabular}{l c c c}
\toprule
方法 & Sys1(reaction--diffusion) & Sys2(pure Neumann 奇异) & Sys3(Laplace 1D+5N) \\
\midrule
BASIC & $O0,L1$ · 26 · 0.048 & $O1,L2$ · 67 · 0.121 & $O0,L2$ · 87 · 0.145 \\
sASM & $O2,L1$ · 18 · 0.041 & $O2,L2$ · 54 · 0.110 & $O2,L1$ · 73 · 0.124 \\
\textbf{sASM+Cheby} & $O3,L1$ · \textbf{11} · 0.042 & $O2,L1$ · \textbf{42} · 0.106 & $O2,L1$ · \textbf{48} · 0.115 \\
\textbf{RAS+GMRES} & $O3,L1$ · 15 · \textbf{0.024} & $O2,L2$ · 40 · \textbf{0.072} & $O3,L2$ · 51 · \textbf{0.102} \\
\bottomrule
\end{tabular}
\end{center}
\vskip0.4em
\begin{itemize}\itemsep0.2em\footnotesize
  \item \textbf{排名三系统一致}:时间 RAS+GMRES $<$ sASM+Cheby $\lesssim$ sASM $<$ BASIC;迭代 sASM+Cheby 最少、BASIC 最多。
  \item RAS+GMRES 领先在良态问题最大(Sys1 $\sim$1.7$\times$),Sys3 最小;代价(内存 $\sim$10$\times$、非对称)与系统无关。
  \item \textbf{反常$+$修复三系统都成立}:Sys2(奇异)BASIC 随 $O$ 仍 $102\to110\to134$↑,sASM $102\to75\to74$↓。
\end{itemize}
\end{frame}
