...
\underbrace{N_c}_{\substack{\text{(B) 着色数(重叠重复计数)}\\ \delta\uparrow\Rightarrow\uparrow}}
\]
\vskip-0.3em
{\footnotesize 由 $\lambda_{\min}\!\ge\! C_0^{-2}$(稳定分解)与 $\lambda_{\max}\!\le\!\omega N_c$(着色 $+$ 局部稳定性)相除得到;推导见附录。精确解时 $\omega{=}1$。底层更细的强化 Cauchy--Schwarz 形式为 $\omega(\rho(\mathcal E){+}1)$,$\rho(\mathcal E){+}1\le N_c$。}
\begin{center}
\begin{tabular}{l ccc c}
\toprule
...
\appendix
\begin{frame}{附录:条件数上界 $\kappa\le C_0^2\,\omega\,N_c$ 的推导(T\&W 定理 2.7)}
\framesubtitle{加性算子 $P_{\text{ad}}=M^{-1}A=\sum_i P_i$,$P_i=R_i^{\top}\tilde A_i^{-1}R_iA$;$a(u,v)=u^{\top}Av$}
\footnotesize
\textbf{假设(2.2,2.4)$+$ 着色.}\
(i) \emph{稳定分解}(Assump. 2.2):每个 $u$ 有 $u=\sum_iR_i^{\top}u_i$ 且 $\sum_i\tilde a_i(u_i,u_i)\le C_0^2\,a(u,u)$;\
(ii) \emph{局部稳定性}(Assump. 2.4):$a(R_i^{\top}u_i,R_i^{\top}u_i)\le\omega\,\tilde a_i(u_i,u_i)$(精确局部解 $\Rightarrow\omega{=}1$);\
(iii) \emph{着色}:用 $N_c$ 色给子域染色,同色支撑不相交($\Rightarrow$ 同色 $a$-正交)。
\vskip0.5em
\textbf{下界(用 i).}\ 由 $u=\sum_iR_i^{\top}u_i$ 得 $a(u,u)=\sum_i a(R_i^{\top}u_i,u)$;每项用 $\tilde a_i$ 的 Cauchy--Schwarz,注意 $\tilde a_i(\tilde A_i^{-1}R_iAu,\cdot)=a(P_iu,u)$:
\[
...
\vskip0.4em
\textbf{上界(用 ii, iii).}\ 按颜色分组 $P_{\text{ad}}=\sum_{c=1}^{N_c}Q_c$,$Q_c=\sum_{i\in I_c}P_i$。同色 $a$-正交 $+$ 局部稳定性 $\Rightarrow\|Q_c\|_a\le\omega$;三角不等式:
\[
\|P_{\text{ad}}\|_a\le\sum_{c=1}^{N_c}\|Q_c\|_a\le N_c\,\omega\quad\Rightarrow\quad\boxed{\lambda_{\max}(P_{\text{ad}})\le\omega N_c}.
\]
\textbf{合并.}\ $\kappa=\lambda_{\max}/\lambda_{\min}\le C_0^2\,\omega\,N_c$。\ 更细可用强化 Cauchy--Schwarz(Assump. 2.3)得 $\lambda_{\max}\le\omega(\rho(\mathcal E){+}1)$,且 $\rho(\mathcal E){+}1\le N_c$。\ 精确子域解 $\omega{=}1$;ICC 不精确 $\omega{>}1$ 且作用于更大重叠块上增大——因素 A。
\end{frame}
