...
\setbeamertemplate{footline}{\hfill\footnotesize\color{Gray}\ifnum\insertframenumber>10 附录\else\insertframenumber\,/\,10\fi\hspace{0.4cm}\vspace{3pt}}
...
% ===== S3 : the anomaly =====
\begin{frame}{反常现象:overlap 增大,CG 迭代次数上升}
\framesubtitle{方案 BASIC,外层 CG;扫 overlap $O$ 与 ICC 填充 $L$}
\begin{center}
\begin{tabular}{l ccc c}
\toprule
ICC 填充 & $O=0$ & $O=1$ & $O=2$ & 随 $O$ 趋势 \\
\midrule
$L=0$ & 132 & 148 & 179 & \ua\ 上升 \\
$L=1$ & 102 & 102 & 117 & \ua\ 上升 \\
$L=2$ & 87  & 92  & 90  & 近持平 \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}\itemsep0.3em
  \item $L=0$ 列最典型:$132\to148\to179$,与经典理论方向相反。
  \item ICC 填充越高(子域解越精确),现象越弱——已暗示成因之一是\emph{子域求解精度}。
\end{itemize}
\vskip0.4em
\ctx
\end{frame}
...
\textbf{机制.}\ 预条件 CG:$n_{\text{iter}}\sim\tfrac12\sqrt{\kappa}\,\ln(2/\varepsilon)$。抽象加性 Schwarz 上界(Toselli \& Widlund 2005,定理 2.7):
\[
\kappa(M^{-1}A)\ \le\
...
\underbrace{(\rho(\mathcal E)+1)}_{\substack{\text{(B) 重叠着色}\le N_c\\ \delta\uparrow\Rightarrow\uparrow}}
\]
\vskip-0.3em
{\footnotesize 由 $\lambda_{\min}\!\ge\! C_0^{-2}$(稳定分解)与 $\lambda_{\max}\!\le\!\omega(\rho(\mathcal E){+}1)$(强化 Cauchy--Schwarz $+$ 局部稳定性)相除得到;推导见附录。精确解时 $\omega{=}1$。}
\begin{center}
\begin{tabular}{l ccc c}
\toprule
...
方案 & it & s & it & s & it & s \\
\midrule
BASIC & 132 & 0.20 & 148 & 0.26 & 179 & 0.32 \\
sASM  & 132 & 0.21 & 104 & 0.18 & 103 & 0.18 \\
\bottomrule
\end{tabular}
\end{center}
\vskip0.2em
{\footnotesize $132\to104\to103$,overlap 恢复有效;$O{=}2$ 处迭代与时间均近乎减半。时间为 3 次最优,负载机器仅供参考。}
\end{column}
\end{columns}
\vskip0.4em
\ctx
...
\appendix
\begin{frame}{附录:条件数上界 $\kappa\le C_0^2\,\omega\,(\rho(\mathcal E){+}1)$ 的推导}
\framesubtitle{加性算子 $P_{\text{ad}}=M^{-1}A=\sum_i P_i$,$P_i=R_i^{\top}\tilde A_i^{-1}R_iA$;$a(u,v)=u^{\top}Av$}
\footnotesize
\textbf{三条假设.}\
(i) \emph{稳定分解}:每个 $u$ 有 $u=\sum_iR_i^{\top}u_i$ 且 $\sum_i\tilde a_i(u_i,u_i)\le C_0^2\,a(u,u)$;\
(ii) \emph{强化 Cauchy--Schwarz}:$a(P_iu,P_ju)\le\epsilon_{ij}\,a(P_iu,P_iu)^{1/2}a(P_ju,P_ju)^{1/2}$,$\mathcal E{=}(\epsilon_{ij})$;\
(iii) \emph{局部稳定性}:$a(R_i^{\top}u_i,R_i^{\top}u_i)\le\omega\,\tilde a_i(u_i,u_i)$(精确局部解 $\Rightarrow\omega{=}1$)。
\vskip0.5em
\textbf{下界(用 i).}\ 由 $u=\sum_iR_i^{\top}u_i$ 得 $a(u,u)=\sum_i a(R_i^{\top}u_i,u)$;对每项用 $\tilde a_i$ 的 Cauchy--Schwarz,注意 $\tilde a_i(\tilde A_i^{-1}R_iAu,\cdot)=a(P_iu,u)$:
\[
...
\]
$\Rightarrow a(P_{\text{ad}}u,u)\ge C_0^{-2}a(u,u)$,即 $\boxed{\lambda_{\min}(P_{\text{ad}})\ge C_0^{-2}}$。
\vskip0.4em
\textbf{上界(用 ii, iii).}\ 记 $x_i=a(P_iu,P_iu)^{1/2}$;又 $a(P_iu,P_iu)\le\omega\,a(P_iu,u)$(由 iii):
\[
\|P_{\text{ad}}u\|_a^2=\sum_{i,j}a(P_iu,P_ju)\overset{(ii)}{\le}x^{\top}\mathcal E x\le\rho(\mathcal E)\sum_i x_i^2\overset{(iii)}{\le}\omega\,\rho(\mathcal E)\,a(P_{\text{ad}}u,u)
\]
由 $a(P_{\text{ad}}u,u)\le\|P_{\text{ad}}u\|_a\|u\|_a$ 得 $\|P_{\text{ad}}\|_a\le\omega\rho(\mathcal E)$;计入对角/粗项,$\boxed{\lambda_{\max}\le\omega(\rho(\mathcal E){+}1)}$。
\vskip0.4em
\textbf{合并.}\ $\kappa=\lambda_{\max}/\lambda_{\min}\le C_0^2\,\omega\,(\rho(\mathcal E){+}1)$。\ 着色论证给 $\rho(\mathcal E){+}1\le N_c$(同色子域互不重叠)。\ 精确子域解 $\omega{=}1$;不精确(ICC)$\omega{>}1$,且作用于更大的重叠块上 $\omega$ 增大——此即因素 A。
\end{frame}
