...
- **【这对 z 意味着什么 `fig_zfield_exact_vs_icc.png`(`zfield_2d.c`)】** 一次 BASIC apply,2D、r=1:
  **精确 $A_i^{-1}$ 把光滑小-$\lambda$ 模放大很多 → z 是大穹顶(‖z‖=1458);ICC(0) 抓不住这个放大 → 低估 10×(‖z‖=142)**。
  **ICC 的不精确恰好住在光滑全局模上**——这正是收敛最慢的 $\lambda_{\min}$/信息传播模(Fig 10)。
  即**"ICC 不精确"="抓不住光滑全局模"="全局模收敛慢"是同一件事**,把因素 A 和信息传播接到了一起。
  **1D 里 ICC=精确,z 完全不变**。
...
### Fig 13 `fig13_seamacc.png` — 精确 vs 非精确:边界误差在缝处的累积(`seamacc.c`)
- **【对应问题】** 2D Laplace,$64^2$,$4{\times}4$,$O{=}2$;BASIC-精确 / BASIC-ICC / sASM-ICC 三条。
- **【回答"Fig 3 能不能分析边界误差累积"】** **不能直接看出**。**Fig 3 只给全局残差范数的收敛差距(23/102/67)= 症状**;
  它不告诉你误差**积在哪**。要"边界误差累积"得看**空间**:Fig 4 的热力图,或这里的**缝处残差占比**量化。
- **【坐标/参数】**
  - **上排 = $k{=}15$ 的残差场热力图**(三方法,**共享对数色标**,白点线=子域缝):
    **精确已收敛(全黑)**、**BASIC-ICC 残差卡在缝上(亮条纹)**、sASM-ICC 较弱且不锁在缝。
  - (a) = relres vs 迭代(同 Fig 3 的三条):精确 23、BASIC-ICC 102、sASM-ICC 68。
  - (b) = **缝处残差占比** $\|r\|^2_{\text{overlap}}/\|r\|^2$ vs 迭代(overlap = $m_k>1$ 的区域)。
- **【结论】**
  - **BASIC 把残差困在过度计数的缝里**(占比 ~0.5–0.8);**精确解时这无害(23 步清掉),ICC 时这团被过度计数的误差成了瓶颈(102 步)**。
  - **sASM 不过度计数缝 ⟹ 残差不被困**(占比恒 ~0.3)⟹ 清得快(68 步)。
  - **这才是"精确 vs 非精确边界误差累积"**:差别不在"残差是否经过缝"(都经过),而在**非精确解在缝处的误差被 BASIC 的过度计数困住、清不掉**;sASM 解开这个困。信息传播(误差从边界往里清)的快慢由此决定。
